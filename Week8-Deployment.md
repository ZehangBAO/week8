...
In VS Code's version control interface, choose `Publish to GitHub` and select the private repository option. Your VS Code should already be linked to your GitHub account (Week 2, refer to [https://code.visualstudio.com/docs/sourcecontrol/github](https://code.visualstudio.com/docs/sourcecontrol/github)).
...
Login to your Docker Hub on the web. From the home page ([https://hub.docker.com/](https://hub.docker.com/)), click `Create a repository`. Give the new repository a name, e.g., `book_app_backend`. Select the option to make the repository public.
...
- If this is the first time you use AWS services, you might need to change the `Inbound rules` in the default security group to allow incoming requests to your app. By default, the default security group in AWS does not allow all inbound traffic. In your ECS service created earlier, click on `Configuration and networking`, and click on the name of the security group, e.g., `sg-64b6b7f0 - default`. In the security group, click `Edit inbound rules`, and ensure the following values are entered before clicking `Save rules`: 
  - `Type`: select `Custom TCP`.
  - `Port range`: enter `8000`.
  - `Source`: `Custom`, and select `0.0.0.0/0` for `CIDR blocks` to enable public access to your container.
  - The above steps ensure that external traffic can reach your container on the specified port `8000`.
